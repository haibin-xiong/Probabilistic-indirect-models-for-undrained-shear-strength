...
for i in range(7):
    for j in range(5):
        ...
        ax.vlines(ori_i_j.iloc[:, 3], ori_i_j.iloc[:, 0], ori_i_j.iloc[:, 2],
                  color=colors['Ori.'], alpha=0.3, linewidth=1, zorder=1)
        ...
        ax.scatter(ori_i_j.iloc[:, 3], ori_i_j.iloc[:, 1], color=colors['Ori.'], s=120, marker='o',
                   edgecolors='black', linewidth=1, alpha=0.9, label='Ori.' if i == 0 and j == 0 else '', zorder=2)
