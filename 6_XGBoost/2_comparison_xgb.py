...
for j in range(5):
    ori_dfs.append(pd.read_excel('ori_predictions_xgb.xlsx', sheet_name=f"fold_{j+1}"))
    ext_dfs.append(pd.read_excel('ext_predictions_xgb.xlsx', sheet_name=f"fold_{j+1}"))
    mice_dfs.append(pd.read_excel('mice_predictions_xgb.xlsx', sheet_name=f"fold_{j+1}"))
    mf_dfs.append(pd.read_excel('mf_predictions_xgb.xlsx', sheet_name=f"fold_{j+1}"))
...
for i in range(7):
    for j in range(5):
        ...
        ax.vlines(ori_i_j.iloc[:, 3], ori_i_j.iloc[:, 0], ori_i_j.iloc[:, 2],
                  color=colormap(0), alpha=0.3, linewidth=2, zorder=1)
        ax.vlines(ext_i_j.iloc[:, 3], ext_i_j.iloc[:, 0], ext_i_j.iloc[:, 2],
                  color=colormap(2), alpha=0.3, linewidth=2, zorder=1)
        ax.vlines(mice_i_j.iloc[:, 3], mice_i_j.iloc[:, 0], mice_i_j.iloc[:, 2],
                  color=colormap(4), alpha=0.3, linewidth=2, zorder=1)
        ax.vlines(mf_i_j.iloc[:, 3], mf_i_j.iloc[:, 0], mf_i_j.iloc[:, 2],
                  color=colormap(6), alpha=0.3, linewidth=2, zorder=1)

        ax.scatter(ori_i_j.iloc[:, 3], ori_i_j.iloc[:, 1], color=colormap(0), s=150, marker='s',
                   edgecolors='black', linewidth=2, label='Ori.' if i == 0 and j == 0 else '', zorder=2)
        ax.scatter(ext_i_j.iloc[:, 3], ext_i_j.iloc[:, 1], color=colormap(2), s=150, marker='o',
                   edgecolors='black', linewidth=2, label='MN' if i == 0 and j == 0 else '', zorder=2)
        ax.scatter(mice_i_j.iloc[:, 3], mice_i_j.iloc[:, 1], color=colormap(4), s=150, marker='<',
                   edgecolors='black', linewidth=2, label='MICE' if i == 0 and j == 0 else '', zorder=2)
        ax.scatter(mf_i_j.iloc[:, 3], mf_i_j.iloc[:, 1], color=colormap(6), s=150, marker='>',
...
# Save the figure
output_path = os.path.join(folder.parent, 'results', 'xgb_comparison.png')
