...
train_NRMSE = [metrics_train["wCI"].values[i * 7:(i + 1) * 7] for i in range(4)]
val_NRMSE = [metrics_val["wCI"].values[i * 7:(i + 1) * 7] for i in range(4)]
test_NRMSE = [metrics_test["wCI"].values[i * 7:(i + 1) * 7] for i in range(4)]
...
ax.set_ylabel(r'$\mathit{w}_{\text{CI}}$')
...
# Show the plot
plt.tight_layout()
output_path = os.path.join(folder.parent, 'results', 'wCI_bar_comparison.png')
