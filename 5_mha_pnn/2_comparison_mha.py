...
for i in range(7):
    for j in range(5):
        ...
        ax.scatter(ext_i_j.iloc[:, 3], ext_i_j.iloc[:, 1], color=colormap(0), s=150, marker='s',
                   edgecolors='black', linewidth=2, label='EXT' if i == 0 and j == 0 else '', zorder=2)
        ax.scatter(mice_i_j.iloc[:, 3], mice_i_j.iloc[:, 1], color=colormap(4), s=150, marker='o',
