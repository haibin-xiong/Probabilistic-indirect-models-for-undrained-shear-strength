...
color_palette_box = [
    ...
    (*colormap(3)[:3], 0.2)  # Second color
]

color_palette_scatter = [colormap(0), colormap(3)]
...
for i in range(4):
    for idx, feature in enumerate(features):
        ...
        if i == 3:
            ...
            ax.set_xticks([0, 1])
            ax.set_xticklabels(['Ori.', 'MF'], fontsize=56, fontweight='bold')
        else:
            ax.set_xlabel('')
            ax.set_xticks([0, 1])
            ax.set_xticklabels([])
...
output_image_path = os.path.join(folder_path.parent, 'results', 'Comparison_Box_MF.png')
