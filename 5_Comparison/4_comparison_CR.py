...
train_NRMSE = [metrics_train["CR"].values[i * 7:(i + 1) * 7] for i in range(4)]
val_NRMSE = [metrics_val["CR"].values[i * 7:(i + 1) * 7] for i in range(4)]
test_NRMSE = [metrics_test["CR"].values[i * 7:(i + 1) * 7] for i in range(4)]
...
# Show the plot
plt.tight_layout()
output_path = os.path.join(folder.parent, 'results', 'CR_bar_comparison.png')
